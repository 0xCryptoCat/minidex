...
function processGoPlusResponse(data: any, chain: string, address: string) {
  try {
    if (isSolana(chain)) {
...
export default async (request: Request): Promise<Response> => {
  ...
  if (!params) {
    return new Response(
      JSON.stringify({ error: 'Missing required parameters: chain, address' }),
      { status: 400, headers }
    );
  }
  ...
  try {
    ...
    return new Response(
      JSON.stringify(securityResponse),
      { status: 200, headers }
    );
